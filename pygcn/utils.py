...
def load_data(path="../data/cora/", dataset="cora"):
    ...
    edges_unordered = np.genfromtxt("{}{}.cites".format(path, dataset),
                                    dtype=np.int32)
    arr = list(map(idx_map.get, edges_unordered.flatten()))
    edges = np.array(list(map(idx_map.get, edges_unordered.flatten())),
                     dtype=np.int32).reshape(edges_unordered.shape)
    print(max(edges.flatten()))
    print(edges.shape)
    print(labels.shape)
...
def load_news_data(path="/home/cyulin/Course/cse547/fp/graph_model/data_v3/", dataset="politifact"):
    ...
    #edges_arr = []
    #for arr in edges_unordered:
        #edges_arr.append(arr)
        #edges_arr.append(arr[::-1])

    #features = sp.csr_matrix(np.ones((idx.shape[0], 1)), dtype=np.float32)
    #edges = np.array(list(map(idx_map.get, edges_unordered.flatten())), 
                              #dtype=np.int32).reshape(edges_unordered.shape)
    edges = edges_unordered
    #edges = np.array(edges_arr, dtype=np.int32)
    #print(max(edges.flatten()))
    #print(edges.shape)
    #print(idx.shape)
